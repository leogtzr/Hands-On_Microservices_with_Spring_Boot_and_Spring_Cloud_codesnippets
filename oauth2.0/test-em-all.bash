...
ACCESS_TOKEN=$(curl --request POST --url 'https://leogtzr.auth0.com/oauth/token' --header 'content-type: application/json' --data '{"grant_type":"password", "username":"${OAUTH_USER}", "password":"${OAUTH_PASSWORD}", "audience":"https://localhost:8443/product-composite", "scope":"openid email product:read product:write", "client_id": "${OAUTH_CLIENT_ID}", "client_secret": "${OAUTH_CLIENT_SECRET}"}' -s | jq .access_token -read)
...
READER_ACCESS_TOKEN=$(curl --request POST \
 ...
 --data '{"grant_type":"password", "username":"${OAUTH_USER}", "password":"${OAUTH_PASSWORD}", "audience":"https://localhost:8443/product-composite", "scope":"openid email product:read", "client_id": "${OAUTH_CLIEND_ID}", "client_secret": "${OAUTH_CLIENT_SECRET}"}' -s | jq -r .access_token)
