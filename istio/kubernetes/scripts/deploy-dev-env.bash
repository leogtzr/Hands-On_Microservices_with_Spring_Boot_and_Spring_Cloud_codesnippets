...
function waitForPods() {
    ...
    actualPodCount=$(minikube kubectl -- get pod -l $labelSelection -o json | jq ".items | length")
...
if minikube kubectl -- -n istio-system get secret istio-ingressgateway-certs > /dev/null ; then
    ...
    minikube kubectl -- create -n istio-system secret tls istio-ingressgateway-certs \
...
minikube kubectl -- wait --timeout=600s --for=condition=available deployment --all

minikube kubectl -- get deployment auth-server product product-composite recommendation review -o yaml | istioctl kube-inject -f - | minikube kubectl -- apply -f -

waitForPods 5 'version=latest'

minikube kubectl -- wait --timeout=120s --for=condition=Ready pod --all
